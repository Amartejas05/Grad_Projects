...
def comp_first(gameState):
    while True:
        ...
        moveHeights[move] += 1
        while gameState[BOARD_SIZE_Y - moveHeights[move]][move] != 0:
            moveHeights[move] += 1
        ...
        while True:
            try:
                move = input("What is your move? (Choose from 1 to 7 )")
                move = int(move)
            except ValueError:
        ...
        print(gameState[BOARD_SIZE_Y - moveHeights[move - 1]][move - 1])
        while gameState[BOARD_SIZE_Y - moveHeights[move - 1]][move - 1] != 0:
            moveHeights[move - 1] += 1
...
def playGame(gameState):
    ...
    global win_count_player
    global win_count_comp
    moveHeights = [0] * BOARD_SIZE_X
    player = COMPUTER_PLAYER
    opponent = HUMAN_PLAYER
    winner = 0
...
if __name__ == "__main__":
    ...
    first_move = sys.argv[3]
    SEARCH_DEPTH = int(sys.argv[4])
    gameState = [[0 for col in range(7)] for row in range(6)]
    with open(input_file, 'r') as f:
        name = f.readlines()
    for i in range(0, 6):
        for j in range(0, 6):
            gameState[i][j] = int(name[i][j])
    playing = True
    while playing:
        if first_move != "computer":
            winner = playGame(gameState)
            if winner == COMPUTER_PLAYER:
                print("Damn! You lost!")
                print("Score of Player is", win_count_player)
                print("Score of Computer is", win_count_comp)
            elif winner == HUMAN_PLAYER and win_count_player > win_count_comp:
